...
int play_amr_init_impl(struct play_decoder *self, play_decoder_cfg_t *cfg)
{
    ...
    g_ABCoreDat.dat[0] = (uint32_t)type;
    g_ABCoreDat.dat[1] = (uint32_t)ch;
    ...
    g_Amr = (struct amr_dec *)g_ABCoreDat.dat[0];
...
play_decoder_error_t play_amr_process_impl(struct play_decoder *self)
{
    ...
    while (1)
    {
        ...
        if (frame_size == 0)
        {
            ...
            {
                RK_AUDIO_LOG_E("Mode %d out of range[0 - %d]\n", mode, sizeof(amrwb_frame_size) - 1);
...
#endif
            {
                RK_AUDIO_LOG_E("Not supported mode %d\n", mode);
                return PLAY_DECODER_DECODE_ERROR;
            }
        }
