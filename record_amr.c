...
record_encoder_error_t record_amr_process(struct record_encoder *self)
{
    ...
    while (1)
    {
        ...
        {
            int read_bytes = amr->input(amr->userdata, (unsigned char *)g_Amr->_buffer_in + i, SAMPLE_PER_FRAME * 2 - i);
        ...
        if (num_enc_bytes < 0)
        {
            RK_AUDIO_LOG_E("AMR-NB encode return %d bytes\n", num_enc_bytes);
            return RECORD_ENCODER_ENCODE_ERROR;
        }
        ...
        int write_bytes = amr->output(amr->userdata, g_Amr->_buffer_out, num_enc_bytes);
        if (write_bytes == -1)
        {
