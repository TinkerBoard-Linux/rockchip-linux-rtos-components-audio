...
static int inner_loop(int ix[samp_per_frame2], int max_bits, gr_info *gi)
{
    ...
    while (step)
    {
        if (!quantize(ix, stepsize, gi->cutoff))
    ...
    while (bits > max_bits)
    {
        quantize(ix, ++stepsize, gi->cutoff);        /* update ix */
        calc_runlen(ix, gi);             /* rzero,count1,big_values */
        bits = count1_bitcount(ix, gi);  /* count1_table selection */
        subdivide(gi);                   /* bigvalues sfb division */
        bigv_tab_select(ix, gi);         /* codebook selection */
...
#endif
{
    ...
    for (gr = 0; gr < mp3->config.mpeg.granules; gr++)
        for (ch = mp3->config.mpeg.channels; ch--;)
        {
            /* setup pointers */
            ix = mp3->l3_enc[gr][ch];
            xr = mp3->mdct_freq[gr][ch];
            gi = &side_info->gr[gr].ch[ch].tt;

            /* calculate absolute and maximum */
            for (i = mp3->config.mpeg.cutoff, xrmax = 0; i--;)
            ...
            gi->scalefac_band_long = mp3->scalefac_band_long;
            gi->cutoff = mp3->config.mpeg.cutoff;
            ...
            for (i = 0; i < mp3->config.mpeg.cutoff; i++)
                if (xr[i] < 0)
                    ix[i] *= -1;
        }

    /* Sort out reservoir at frame end, limit the size and stuff the excess */
    resv_max = DEC_BUFF_LIMIT - mp3->config.mpeg.bits_per_frame;
    if (resv_max > mp3->config.mpeg.resv_limit)
...
void subdivide(gr_info *gi)
{
    ...
    else
    {
        bigvalues_region = 2 * gi->big_values;
        {
            int thiscount, index;
            /* Calculate scfb_anz */
            while (gi->scalefac_band_long[scfb_anz] < bigvalues_region)
                scfb_anz++;

            gi->region0_count = subdv_table[scfb_anz].region0_count;
            thiscount = gi->region0_count;
            index = thiscount + 1;
            while (thiscount && (gi->scalefac_band_long[index] > bigvalues_region))
            {
                thiscount--;
                index--;
            }
            ...
            gi->address1 = gi->scalefac_band_long[gi->region0_count + 1];
            gi->address2 = gi->scalefac_band_long[gi->region0_count
                                                  + gi->region1_count + 2 ];
